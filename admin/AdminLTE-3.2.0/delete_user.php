      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item">
            ...
            <ul class="nav nav-treeview">
              <li class="nav-item">
                <a class="nav-link active" href="all_users.php">
                  <i class="far fa-circle nav-icon"></i>
                  <p>View all users</p>
                </a>
              </li>
                ...
                <table class="table table-head-fixed text-nowrap">
                  ...
                  <?php
                    ...
                    if ($row > 0) {
                        ...
                        echo '
                        <tr class = "usr-"'.$result['id'].'>
                        <td class = "id'.$result['id'].'" >'.$result['id'].'</td>
                        <td class = "name'.$result['id'].'">'.$result['fname'].'</td>
                        <td class = "desc'.$result['id'].'">'.$result['lname'].'</td>
                        <td class = "nprice'.$result['id'].'">'.$result['email'].' <span class="text-danger small">'.$stat.'</span></td>
                        <td class = "nprice'.$result['id'].'">'.$result['user_type'].'</td>
                        <td><button class = "btn btn-danger" onclick = "delete_user('.$result['id'].')">Delete</button></td>
   ...
   delete_user = (id) => {
    if (confirm("Do you really want to delete this user?")) {
        ...
        success: function (response) { 
            $('.usr-'.id).remove();
            toastr.success('Deletetion successful');
